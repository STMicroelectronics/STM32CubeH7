/*********************************************************************************/
/********** THIS FILE IS GENERATED BY TOUCHGFX DESIGNER, DO NOT MODIFY ***********/
/*********************************************************************************/
#include <gui_generated/containers/HomeUIWheelLargeElementsBase.hpp>
#include <texts/TextKeysAndLanguages.hpp>
#include <touchgfx/Color.hpp>
#include "BitmapDatabase.hpp"

HomeUIWheelLargeElementsBase::HomeUIWheelLargeElementsBase()
{
    setWidth(120);
    setHeight(135);

    textAreaBottomlineLarge.setPosition(24, 120, 73, 14);
    textAreaBottomlineLarge.setColor(touchgfx::Color::getColorFrom24BitRGB(52, 75, 103));
    textAreaBottomlineLarge.setLinespacing(0);
    textAreaBottomlineLarge.setTypedText(TypedText(T_HOMEUIWHEELHEATINGUPCENTER));

    textAreaDevicesLarge.setPosition(26, 56, 69, 70);
    textAreaDevicesLarge.setColor(touchgfx::Color::getColorFrom24BitRGB(52, 75, 101));
    textAreaDevicesLarge.setLinespacing(0);
    Unicode::snprintf(textAreaDevicesLargeBuffer, TEXTAREADEVICESLARGE_SIZE, "%s", TypedText(T_SINGLEUSEID104).getText());
    textAreaDevicesLarge.setWildcard(textAreaDevicesLargeBuffer);
    textAreaDevicesLarge.setTypedText(TypedText(T_SINGLEUSEID103));

    textAreaRoomLarge.setPosition(0, 3, 120, 19);
    textAreaRoomLarge.setColor(touchgfx::Color::getColorFrom24BitRGB(52, 75, 101));
    textAreaRoomLarge.setLinespacing(0);
    textAreaRoomLarge.setTypedText(TypedText(T_HOMEUIWHEELBATHROOMSELECTED));

    imageSliderLarge.setXY(0, 35);
    imageSliderLarge.setBitmap(Bitmap(BITMAP_MAIN_SLIDER_TEMP_ID));

    textAreaDegreeLarge.setXY(54, 48);
    textAreaDegreeLarge.setColor(touchgfx::Color::getColorFrom24BitRGB(46, 64, 86));
    textAreaDegreeLarge.setLinespacing(0);
    textAreaDegreeLarge.setTypedText(TypedText(T_SINGLEUSEID105));

    add(textAreaBottomlineLarge);
    add(textAreaDevicesLarge);
    add(textAreaRoomLarge);
    add(imageSliderLarge);
    add(textAreaDegreeLarge);
}

void HomeUIWheelLargeElementsBase::initialize()
{
	
}
