    ...
    updateItemCallback(this, &OvenScreenViewBase::updateItemCallbackHandler)
{
    ...
    background.setBitmap(Bitmap(BITMAP_MAIN_BG_ID));

    imageButtomBar.setXY(0, 192);
    imageButtomBar.setBitmap(Bitmap(BITMAP_BOTTOMBAR_ID));
    imageButtomBar.setAlpha(0);

    containerScreenElements.setPosition(0, 0, 480, 272);

    imageButtomBarWheel.setXY(0, 172);
    imageButtomBarWheel.setBitmap(Bitmap(BITMAP_MAIN_BUNDBAR_BG_ID));
    imageButtomBarWheel.setAlpha(0);
    containerScreenElements.add(imageButtomBarWheel);

    buttonKitchen.setXY(0, 0);
    buttonKitchen.setBitmaps(Bitmap(BITMAP_GO_BACK_KITCHEN_ID), Bitmap(BITMAP_GO_BACK_KITCHEN_ID));
    ...
    Unicode::snprintf(textAreaTemperatureBuffer, TEXTAREATEMPERATURE_SIZE, "%s", TypedText(T_SINGLEUSEID52).getText());
    textAreaTemperature.setWildcard(textAreaTemperatureBuffer);
    textAreaTemperature.setTypedText(TypedText(T_SINGLEUSEID51));
    ...
    Unicode::snprintf(textAreaTimerBuffer1, TEXTAREATIMERBUFFER1_SIZE, "%s", TypedText(T_SINGLEUSEID54).getText());
    textAreaTimer.setWildcard1(textAreaTimerBuffer1);
    Unicode::snprintf(textAreaTimerBuffer2, TEXTAREATIMERBUFFER2_SIZE, "%s", TypedText(T_SINGLEUSEID55).getText());
    textAreaTimer.setWildcard2(textAreaTimerBuffer2);
    textAreaTimer.setTypedText(TypedText(T_SINGLEUSEID53));
    containerScreenElements.add(textAreaTimer);

    buttonTemperature.setXY(0, 192);
    buttonTemperature.setBitmaps(Bitmap(BITMAP_BUTTONOVERLAY_ID), Bitmap(BITMAP_BUTTONOVERLAY_ID));
    buttonTemperature.setAction(buttonCallback);
    containerScreenElements.add(buttonTemperature);

    buttonTimer.setXY(315, 192);
    buttonTimer.setBitmaps(Bitmap(BITMAP_BUTTONOVERLAY_ID), Bitmap(BITMAP_BUTTONOVERLAY_ID));
    buttonTimer.setAction(buttonCallback);
    containerScreenElements.add(buttonTimer);

    buttonOk.setXY(170, 200);
    buttonOk.setVisible(false);
    buttonOk.setBitmaps(Bitmap(BITMAP_OK_BUTTON_ID), Bitmap(BITMAP_OK_BUTTON_PRESSED_ID));
    ...
    scrollWheelOvenProgram.setEasingEquation(EasingEquations::backEaseOut);
...
void OvenScreenViewBase::updateItemCallbackHandler(DrawableListItemsInterface* items, int16_t containerIndex, int16_t itemIndex)
{
