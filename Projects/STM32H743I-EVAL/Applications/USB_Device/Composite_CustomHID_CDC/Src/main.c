...
/** @addtogroup Composite_CustomHID_CDC
  * @{
  */
...
int main(void)
{
  ...
  CUSTOMHID_InstID = USBD_Device.classId;
  
  /* Register the Custom HID  class */
  USBD_RegisterClassComposite(&USBD_Device, USBD_CUSTOM_HID_CLASS, CLASS_TYPE_CHID, CustomHID_EpAdress);
  
  /* Store CDC instance Class ID */
  CDC_InstID = USBD_Device.classId;

  /* Register CDC class first instance */
  USBD_RegisterClassComposite(&USBD_Device, USBD_CDC_CLASS, CLASS_TYPE_CDC, CDC_EpAdress);

  /* Add Custom HID Interface Class */
  if (USBD_CMPSIT_SetClassID(&USBD_Device, CLASS_TYPE_CHID, 0) != 0xFF)
  {
